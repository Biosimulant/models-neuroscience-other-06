# PinskyRinzelModel

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/PinskyRinzelModel)
**Standard**: other
**Authors**: Open Source Brain

## Description

Reduced CA3 cell model from Pinsky & Rinzel 1994


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-pinskyrinzelmodel-pinskyrinzelmodel-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
