# NeuroDataShare

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/NeuroDataShare)
**Standard**: other
**Authors**: Open Source Brain

## Description

Exploring and sharing multi-scale neuroscience data


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-neurodatashare-neurodatashare-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
